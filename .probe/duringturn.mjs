/** Holes WHILE the camera is turning. The sweep measured 500ms after the click and saw 36 water pixels; a
 *  settled map shows 5. If the map is drawn at a FRACTIONAL turn while the discrete facing still drives the
 *  neighbour lookups, that is a real artefact, and it is only visible mid-spin. */
...
const count = () => p.evaluate(() => {
  ...
  for (let n = 0; n < width * height; n++) {
    ...
    if (col >= 0 && row >= 0 && col < g.cols && row < g.rows) {
      ...
      const k = `${f?.tileKey ?? 'BARE'}@h${g.getHeight(col, row)}`
      blame.set(k, (blame.get(k) ?? 0) + 1)
    } else off++
  }
  oc.putImageData(img, 0, 0)
  return { on, off, blame: [...blame].sort((a, c) => c[1] - a[1]).slice(0, 6).map(([k, v]) => `${k}:${v}`).join(' '), png: on > 3000 ? out.toDataURL('image/png') : null }
})
...
for (const turn of [1, 2]) {
  ...
  for (let i = 0; i < 8; i++) {
    const r = await count()
    console.log(`turn ${turn}  +${String(Date.now() - t0).padStart(4)}ms   on-map ${String(r.on).padStart(5)}   off-map ${String(r.off).padStart(5)}   ${r.blame}`)
    if (r.png && !saved) { writeFileSync('/home/visiond/.claude/jobs/beedf1c6/tmp/shots/midturn.png', Buffer.from(r.png.split(',')[1], 'base64')); saved = true }
