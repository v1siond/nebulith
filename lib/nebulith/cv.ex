defmodule Nebulith.CV do
  ...
  defp professional_summary(locale) do
    row =
      from(s in "ProfessionalSummary",
        limit: 1,
        select: %{
          headline: s.headline, tagline: s.tagline, bio: s.bio, highlights: s.highlights,
          headline_es: s.headline_es, headline_it: s.headline_it,
          tagline_es: s.tagline_es, tagline_it: s.tagline_it,
          bio_es: s.bio_es, bio_it: s.bio_it,
          highlights_es: s.highlights_es, highlights_it: s.highlights_it
        }
      )
      |> Repo.one()
  ...
  defp current_roles(locale) do
    from(r in "Role",
      ...
      select: %{
        id: r.id, slug: r.slug, title: r.title, company: r.company, type: r.type,
        description: r.description, title_es: r.title_es, title_it: r.title_it,
        description_es: r.description_es, description_it: r.description_it
      }
    )
    |> Repo.all()
    |> Enum.map(fn r ->
      %{
        id: r.id, slug: r.slug, title: lstr(r, :title, locale), company: r.company,
        type: r.type, description: lstr(r, :description, locale)
      }
    end)
  end
  ...
  defp companies(locale) do
    from(c in "Company",
      ...
      select: %{
        ...
        url: c.url, services: c.services, icon: c.icon,
        tagline_es: c.tagline_es, tagline_it: c.tagline_it,
        description_es: c.description_es, description_it: c.description_it,
        services_es: c.services_es, services_it: c.services_it
      }
    )
    |> Repo.all()
    |> Enum.map(fn c ->
      %{
        id: c.id, slug: c.slug, name: c.name, tagline: lstr(c, :tagline, locale),
        description: lstr(c, :description, locale), url: c.url,
        services: ljson(c, :services, locale), icon: c.icon
      }
    end)
  end
  ...
  defp featured_projects(locale) do
    from(p in "Project",
      ...
      select: %{
        ...
        impact: p.impact, techStack: p.techStack, status: p.status, links: p.links,
        name_es: p.name_es, name_it: p.name_it, tagline_es: p.tagline_es, tagline_it: p.tagline_it,
        description_es: p.description_es, description_it: p.description_it,
        impact_es: p.impact_es, impact_it: p.impact_it
      }
    )
    |> Repo.all()
    |> Enum.map(fn p ->
      %{
        id: p.id, slug: p.slug, name: lstr(p, :name, locale), tagline: lstr(p, :tagline, locale),
        description: lstr(p, :description, locale),
        impact: if(p.impact, do: lstr(p, :impact, locale)),
        techStack: p.techStack, status: p.status, links: p.links
      }
    end)
  end
  ...
  defp tech_stack(locale) do
    ...
    by_cat =
      from(t in "Technology", order_by: [asc: t.order], select: %{name: t.name, categoryId: t.categoryId})
      |> Repo.all()
  ...
  defp work_experience(locale) do
    from(e in "WorkExperience",
      ...
      select: %{
        ...
        highlights: e.highlights, skills: e.skills,
        title_es: e.title_es, title_it: e.title_it,
        description_es: e.description_es, description_it: e.description_it,
        highlights_es: e.highlights_es, highlights_it: e.highlights_it
      }
    )
    |> Repo.all()
    |> Enum.map(fn e ->
      %{
        id: e.id, title: lstr(e, :title, locale), company: e.company,
        description: lstr(e, :description, locale), startDate: e.startDate, endDate: e.endDate,
        current: e.current,
