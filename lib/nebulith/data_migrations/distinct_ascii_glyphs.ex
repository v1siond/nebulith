defmodule Nebulith.DataMigration.DistinctAsciiGlyphs do
  @moduledoc """
  ...
  same picture as a rose is not a catalog, it is a coincidence.
  ...
  The glyph is DB data, so it changes here and never in the frontend. The bake source
  ...
  def run do
    ...
    Logger.info("[data_migrate] distinct ascii glyphs (#{updated} rows set)")
    :ok
  end
