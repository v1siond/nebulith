defmodule Nebulith.Catalog.GeneratorSource do
  ...
  # THE REGIONS A BIOME IS MADE OF, and they are ITS OWN, never one set of five wearing nine sets of colours.
  #
  # `docs/REGIONS.md` §1.1: the set belongs to the BIOME. *"the zones of a meadow aren't the same of a swamp
  # ... we need to have logical sections per BIOM, not generic ones repeated across all of them, and each
  # region should create a distinct template, different layout, different everything"*.
  #
  # THESE LIVE HERE BECAUSE THE SEEDER WRITES THE WHOLE `config` COLUMN. They were authored in data
  # migrations that ran `UPDATE generators SET config = jsonb_set(config, '{subZones}', ...)` on top of the
  # seeder's output, and `seed/0` writes that column from its own literal, so every re-seed put the generic
  # wood's five regions back on all nine biomes. Ten migrations' worth of region work disappeared that way,
  # more than once, and nothing in the code had changed. One fact, one owner.
  #
  ...
  @biome_regions %{
    ...
    # own blocking plant and belongs to `understory` alone.
    "jungle" =>
      {"scatter",
       [
         %{
    ...
    # map had the `rings` arrangement and the generic wood's five regions inside it.
    "volcanic" =>
      {"rings",
       [
         %{
  ...
  # A biome's OWN regions, or the generic wood's five for one that has not been authored yet. The authored
  # sets carry their own species and tones, so nothing is looked up for them and nothing is overwritten.
  defp biome_regions(env) do
    case Map.get(@biome_regions, env.key) do
      {_arrangement, regions} -> regions
      nil -> @wild_regions
    end
  end

  # An environment fills a field the region left open. It never overwrites one the region states: a set
  # authored for THIS biome has already answered, and a lookup keyed by the generic wood's region names has
  # nothing to say about a `crater` or a `bamboo` stand anyway.
  ...
  defp region_layout(env) do
    case Map.get(@biome_regions, env.key) do
      {arrangement, _regions} -> arrangement
      nil -> "scatter"
    end
  end
  ...
  All FOUR categories are the SAME environment list, so they are generated as a cross product rather than
