defmodule Nebulith.Catalog.BuildingCompositions do
  ...
  **Roof z-width collapse (#32).** A ROOF is authored as ONE depth-spanned block PER COLUMN, not one
  ...
  same `%{footprint: …, cells: […]}` shape as the authored compositions, because it is what authors them, `definitions/0` calls this eleven times, so the existing composition tests are the proof that this
  ...
  # solid roof; 2D collapses depth onto the front face (MAP-MODEL §2-3). EVERY building, hospital included, # is box-built this way; identity (plaster + green roof, slate gable, …) rides in as the builder's material
  # args.
  ...
  # DEPTH along the +row (south) axis via `depth`/`depthDir`, carrying its gable-step HEIGHT as `scaleY`, # instead of one cell per (col,row). Anchored at the BACK row (dy=0) so the +row (`left-down`) span reaches
