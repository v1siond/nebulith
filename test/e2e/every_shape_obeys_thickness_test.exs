defmodule Nebulith.E2E.EveryShapeObeysThicknessTest do
  ...
  *"I can select any part(tile) of the tree, then modify it's thickness or width or whatever to make it
  ...
  test "a round tile thinned in the panel draws thinner", %{session: session} do
    # THE ROUND CASE ONLY, and that is deliberate. The square case has its own gate already,
    # `Phase03ShapesTest`, "is thinned by its thickness, never by its width or its depth". Adding a square
    # control here meant measuring a lamp post with a pixel sweep that finds the widest lit run anywhere on
    # the canvas, and on a lamp post that is its GLOW, not its post, so the control reported a defect that
    # was not there. A measurement that cannot see the thing it is pointed at is worse than no measurement.
    wide = painted_width(session, "tree_round", nil)
    thin = painted_width(session, "tree_round", 0.3)

    assert wide > 0, "the tree painted nothing at all, so this measures nothing"

    assert thin < wide,
           """
           a round tree crown thinned to 0.3 of its cell painted #{thin}px, and untouched it painted
           #{wide}px, so moving the Thickness control changed nothing about it. A round tile is a cuboid with
           a clip over it and honours every setting a square one does.
           """
  end
