defmodule Nebulith.CatalogTilesTest do
  ...
  test "upsert_tile/1 + list_tiles_for/1 round-trips a tile", %{tileset: ts} do
    {:ok, _} = Catalog.upsert_tile(%{tileset_id: ts.id, label: "leaf_center", color_role: "canopy", image_url: "/tiles/ascii/leaf_center.png", blocking: false, height: 1})
  ...
  test "upsert_tile/1 is idempotent on (tileset,label)", %{tileset: ts} do
    ...
    {:ok, _} = Catalog.upsert_tile(%{tileset_id: ts.id, label: "trunk", color_role: "trunk", image_url: "/tiles/ascii/trunk.png"})
  ...
  test "upsert_composition_with_cells/2 stores footprint + cells" do
    {:ok, comp} = Catalog.upsert_composition_with_cells(
