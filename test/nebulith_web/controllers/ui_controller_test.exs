defmodule NebulithWeb.UiControllerTest do
  ...
  describe "seeded" do
    ...
    test "saving a game's bars FORKS the default, so other games keep theirs", %{conn: conn, game_id: game} do
      bars = [%{"name" => "Vehicle", "rows" => 1, "cols" => 3, "condition" => %{"when" => "vehicle"}, "slots" => []}]

      saved = json_response(put(conn, ~p"/api/ui?game=#{game}", %{"bars" => bars}), 200)["data"]["profile"]
    ...
    test "a bar can be added, reordered and removed in one save", %{conn: conn, game_id: game} do
      ...
      assert ["A", "B"] = json_response(put(conn, ~p"/api/ui?game=#{game}", %{"bars" => two}), 200)["data"]["profile"]["bars"] |> Enum.map(& &1["name"])

      swapped = [%{"name" => "B", "slots" => []}]
      assert ["B"] = json_response(put(conn, ~p"/api/ui?game=#{game}", %{"bars" => swapped}), 200)["data"]["profile"]["bars"] |> Enum.map(& &1["name"])
    end

    test "a saved bar keeps its condition, its shape and its slots", %{conn: conn, game_id: game} do

      bars = [%{
        "name" => "Vehicle", "rows" => 2, "cols" => 5,
        "settings" => %{"buttonPx" => 52, "showText" => true},
        "condition" => %{"when" => "vehicle", "id" => "cart"},
        "slots" => [%{"slot" => 0, "refKind" => "ability", "refKey" => "fire_slash"}]
      }]

      [bar] = json_response(put(conn, ~p"/api/ui?game=#{game}", %{"bars" => bars}), 200)["data"]["profile"]["bars"]
    ...
    test "moving an element saves its placement without touching the others", %{conn: conn, game_id: game} do
      moved = [%{"elementKey" => "vitals", "form" => "Desktop", "placement" => %{"a" => "TR", "x" => 40, "y" => 12}}]

      elements = json_response(put(conn, ~p"/api/ui?game=#{game}", %{"elements" => moved}), 200)["data"]["profile"]["elements"]
