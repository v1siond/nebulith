...
//
// `isWalkable(label)` was removed from `cellLabels.ts` on 2026-09-06, and its own note says why: it hardcoded a
// walkability table in the frontend, it claimed a ROOF was walkable, and it duplicated data the backend already owns
// and serves as `tiles.blocking` and `composition_cells.walkable`. It had no runtime callers.
//
// So there is nothing here to re-point at: the question "is this label walkable?" is not one the frontend answers any
// more. Collision now rides on the placed tile, is per-view, and is covered by the collision and composition suites.
// `isGroundContact`, `autotileLabel` and `TREE_MASS_FAMILY` are still real and still tested above.
