 ...
 *
 * Three bands, plus a per-tile floor: - FAR → fully opaque. A building you are nowhere near is a solid building. -
 * APPROACH → eases translucent as you close in, so you can read the facade and find its door. - INSIDE → the deep
 * reveal: you are under the roof, the shell drops right back so the room reads. - `minAlpha` is a per-tile SETTING
 * (backend data, not a name check in the renderer): the DOOR carries a high one so it stays opaque and obvious while
 * the wall around it fades away.
 */
...
describe('revealAlpha — far is solid, approach eases, inside reveals', () => {
  ...
  // The old smoothstep was far too gradual — at 4 cells out it computed ~0.96, i.e. no visible change at all.
  // Standing NEAR a building must actually make it see-through, not almost-solid.
