 ...
 *
 * ROOT CAUSE this pins: a FLOOR is a `type:'floor'` GridAsset carrying only a `tileKey` (its ground kind), * NO label and empty `art: ['']`. ASCII resolves a KIND to nothing (`ASCII_STYLE.map` is empty by design →
...
describe('kindTileImage, the baked ASCII image for a KIND-identified (label-less) tile', () => {
  ...
  // One engine, N styles: a kind IS a label in the catalog, so it resolves the
