...
describe('loadTilesetsFromBackend, installs the new /api/tilesets shape', () => {
  ...
  test('ascii bush tile gets a resolvable image src derived from its image_url', async () => {
    ...
    //
    // This asked for an absolute URL while the engine was served from a different origin than the API,
    // where a bare path would have resolved against the wrong host. One origin serves both now, so what
    // is worth defending is that the src resolves against the SERVING origin: root-relative, never a
    // document-relative fragment, which would break the moment the page is /games/42 instead of /games.
    expect(typeof bush.image).toBe('string')
    expect(bush.image?.startsWith('/')).toBe(true)
