...
function makeGrid(assetOver: Partial<GridAsset> = {}): IsometricGrid {
  ...
  const asset: GridAsset = {
    ...
    color: MAGENTA, height: 1, width: 3, height: 3, depth: 3, placedAt: T0, ...assetOver,
  } as GridAsset
...
function topCanvas(grid: IsometricGrid, clock: number, style: Style): Canvas {
  const cv = H.makeCanvas(W, H2)
  const ctx = cv.getContext('2d') as unknown as CanvasRenderingContext2D
  renderTopView({ ctx, w: W, h: H2, grid, player: PLAYER, zoom: 4, selectedCells: new Set(), connectors: [], connectorMode: false, camOffset: { x: 0, y: 0 }, entities: [], enemyCombat: new Map(), hitMarkers: [], now: clock, quests: [], dayNight: 'day', style })
  return cv
}
...
beforeAll(async () => {
  ...
  await H.warm([SRC])
  // The top view reads ground terrain straight from styleCatalog('ascii').terrain (normally DB-seeded, `{}` in tests).
