...
import { EDITOR_GUIDES, guideFor } from '@/components/shell/editorGuides.data'

/** The files that draw the controls the guides point at. */
const SOURCES = [
  'components/editorChrome.tsx',
  ...
  'components/editorInspector.tsx',
  'components/shell/ArtStyleControl.tsx',
  'components/editorConfig.ts',
  'routes/templates.tsx',
]

const haystack = SOURCES.map(f => readFileSync(join(process.cwd(), 'game', f), 'utf8')).join('\n')
