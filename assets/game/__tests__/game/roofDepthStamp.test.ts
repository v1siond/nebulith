 ...
 * and, ROTATED by the building's rotation (rotateDepthDir), `settings.spanAxis`, alongside the existing
...
describe('stampRun copies a roof cell depth-span onto the placed asset, rotated by the building rotation', () => {
  ...
  test('rotation 0 (south): keeps the authored +row span (depth=4, spanAxis=left-down) and scaleY height', () => {
  ...
  test('rotation 3 (east): span direction follows to +col (left-down → right-down)', () => {
    ...
    const roof = grid.assets.find(a => a.label === 'roof')!
    expect(roof.spanAxis).toBe('right-down')
  })

  test('a cell with NO depth is untouched, no depth/spanAxis on the placed asset (byte-identical to before)', () => {
    ...
    expect(a.spanAxis).toBeUndefined()
  })
})
