...
export function applyStageToGrid(
  ...
  for (let r = 0; r < grid.rows; r++) {
    for (let c = 0; c < grid.cols; c++) {
      ...
      if (kind) {
        ...
        // THE WATER'S OWN HEIGHT IS THE TILE'S. It used to be overridden here to `dug - 0.25`, which put the
        // surface a quarter of a block under the bank: measurably below it, visually flush with it, so the
        // channel had no visible edge. The tile carries 0.35 stacked from the cell's BOTTOM face now, so in a
        // one-deep channel the surface lands 0.65 under the bank and the cut shows the whole way along.
        //
        // Nothing is invented here any more: the depth of the channel is the generator's, the height of the
        // water is the tile's, and this file just applies them.
      }
  ...
  for (const a of paint.assets) {
    ...
    // all tiles stack", no floorStackLift, the prop just lands on top of what's there.
  ...
  for (const b of stage.buildings) {
    ...
    const cells = stampBuildingKind(grid, b.kind, b.col, anchorRow, stage.zone, b.facing, material, roofColor, wallColor, roofTile)
  ...
  // what is already there, "floors are tiles, all tiles stack".
  // THE TREE'S OWN GREEN, when the generator picked one. Absent leaves the composition's authored colour,
  // which is every tree that came before this.
  for (const t of stage.trees ?? []) stampComposition(grid, t.kind, t.col, t.row, stage.zone, t.variant, 0, { leafColor: t.leafColor })
  ...
  //    terrain for now. I want to load test performance, we can find better ways to optimize after the basics
  //    are done, right now that optimization is just getting in the way."
  //
  // What it does and why it is worth having LATER: it merges a run of identical adjacent floor tiles into one
  // asset carrying `depth`, measured at 1600 floor tiles down to 93 on a town, 2464 to 116 on a city, 2400 to
  // 163 on a forest.
  //
  // Why it is in the way NOW: one tile picture is STRETCHED across the whole run, so a river came out as long
  // smeared bars instead of cells, and the autotile border pieces at a body of water's edge were swallowed
  // into the run beside them. It also spans many camera depths under one sort key, which is the occlusion
  // objection that switched it off once before, on 2026-07-27.
  //
  // `compressGround()` itself is untouched and still tested. Turning it back on is uncommenting this line,
