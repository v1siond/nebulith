...
export function assetKind(asset: { type: string; label?: string; tileKey?: string }): ElementKind {
  ...
  if (label) {
    ...
    // paint their OWN tile (🪨 stone / 🧱 brick / 🟫 wood / ⬜ plaster wall · ⬛ slate roof · 🟦 water / 💧 jet), // NOT the coarse whole-object wall(🧱)/roof/fountain(⛲) emoji. Returning the unmapped 'ground' routes ISO
...
export function tilesForStyle(styleId: string): Record<TileCategory, TileDef[]> {
  ...
  // `extra` carries the DB tile's BLOCK height + settings so the palette tile FULLY describes the DB tile, // the brush then seeds a painted asset from it and a painted tile matches the generator's version. BOTH
