...
/** A one-click starting template: a ready frame set + timing the author can then tweak. */
...
/** The doc's leaf-wind example: static → right → left → right. Wind out of the box. */
export const WIND_SWAY: AnimPreset = {
  id: 'wind-sway',
  name: 'Wind sway',
  frames: [
    { dx: 0, dy: 0 },
    { dx: 0.15, dy: 0, rot: 0.06 },
    { dx: -0.15, dy: 0, rot: -0.06 },
    { dx: 0.15, dy: 0, rot: 0.06 },
  ],
  durationMs: 1200,
...
/** flower-sway as a ready 3-frame sine sway (rest → lean right → lean left). */
export const FLOWER_SWAY: AnimPreset = {
  id: 'flower-sway',
  name: 'Flower sway',
  frames: [
    { dx: 0, dy: 0 },
    { dx: 0.08, dy: -0.02, rot: 0.12 },
    { dx: -0.08, dy: -0.02, rot: -0.12 },
  ],
  durationMs: 1600,
  delayMs: 200,
...
/** lamp-flicker as a quick scale pulse. */
export const LAMP_FLICKER: AnimPreset = {
  id: 'lamp-flicker',
  name: 'Lamp flicker',
  frames: [
    { dx: 0, dy: 0, scale: 1 },
    { dx: 0, dy: 0, scale: 1.12 },
    { dx: 0, dy: 0, scale: 0.96 },
  ],
  durationMs: 650,
  delayMs: 0,
  ease: 'linear',
...
/** bush-rustle as a small jitter. */
export const BUSH_RUSTLE: AnimPreset = {
  id: 'bush-rustle',
  name: 'Bush rustle',
  frames: [
    { dx: 0, dy: 0 },
    { dx: 0.06, dy: -0.03, rot: 0.04 },
    { dx: -0.06, dy: 0.02, rot: -0.04 },
    { dx: 0.04, dy: 0 },
  ],
  durationMs: 2000,
  delayMs: 300,
...
/** The starting templates offered in the author panel (wind first — the headline use case). */
export const CELL_ANIM_PRESETS: readonly AnimPreset[] = [
  WIND_SWAY,
  FLOWER_SWAY,
  LAMP_FLICKER,
  BUSH_RUSTLE,
