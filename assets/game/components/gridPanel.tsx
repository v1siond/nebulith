 ...
 * moves are Alexander's, and the second reverses the first with his reason:
 *
 * 2026-09-10 (a): *"we should add an option in the main sidebar related specifically to the grid ...
 * outside of the template generation"*.
 *
 * 2026-09-10 (b): *"grid UX is not good, my decision to move the grid out of the world section was worse,
 * I feel like is not entirely connected to the section still, but it's way better to have that close by to
 * generate stuff on demand, it also adds extra complexity, like now what happens if user generated a world,
 * then changes the grid settings after?? it just complicates things ... maybe we move the dimensions back to
 * the section they were before ... let's keep thickness out of the new world section, I think it makes sense
 * close to rotate and range, which are grid properties too in a way ... but we keep grid size inside the
 * world section"*.
 *
 ...
 *
 * That also answers his *"what happens if user generated a world, then changes the grid settings after"*:
 ...
 *
 * Alexander, 2026-09-10: *"It doesn't allow me to delete number"*. The old control ran `parseInt` on every
 ...
 *
 * Alexander, 2026-09-10: *"I think it makes sense close to rotate and range, which are grid properties too
 * in a way"*. He is right about the kind of thing it is: it rebuilds nothing and touches no cell, it only
